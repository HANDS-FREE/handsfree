...
class RadianTurn(object):
    def __init__(self, robot_name=''):
        ...
        self.tolerance_radian = rospy.get_param('~tolerance', 0.08)    # 0.08 = 5/360*2*pi
        self.speed_radian = rospy.get_param('~speed_radian', 0.174)  # 0.174 = 10/360*2*pi
...
if __name__ == '__main__':
    ...
    t_rad = rospy.get_param('~t_rad', 0.0)
    if t_rad == 0.0:
        rospy.logerr('no target radian set!')
        exit(-1)
    rospy.loginfo('try to turn %f radian'%t_rad)
    RadianTurn().turn_to_target(radian_to_turn=t_rad)
