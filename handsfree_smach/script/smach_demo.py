...
def pub_initial_pos(frame_id, x, y, yaw):
    q = tf.transformations.quaternion_from_euler(0,0,yaw)
    pub = rospy.Publisher('/initialpose', PoseWithCovarianceStamped, queue_size=10)
    initial_pos = PoseWithCovarianceStamped()
    initial_pos.pose.pose.position.x = x
    initial_pos.pose.pose.position.y = y
    initial_pos.pose.pose.orientation.x = q[0]
    initial_pos.pose.pose.orientation.y = q[1]
    initial_pos.pose.pose.orientation.z = q[2]
    initial_pos.pose.pose.orientation.w = q[3]
    initial_pos.header.frame_id = frame_id
    initial_pos.header.stamp = rospy.Time.now()
    pub.publish(initial_pos)


def gen_back_goal(frame_id, x, y):
    goal = MoveBaseGoal()
...
class MoveBack(smach.State):
    ...
    def execute(self, ud):
        goal = gen_back_goal('base_link', -0.5, 0)
